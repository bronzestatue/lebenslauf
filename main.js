// Publications data (updated with JSON content)
const publications = [
  {
...
// New: Experiences data from experiences.json (translated into JavaScript)
const experiences = [
  {
    "title": "IT Consultant",
...
// Certifications data from certifications.json
...
function createPublicationElement(publication) {
  ...
  div.innerHTML = `
    <div class="publication-header">
      ...
      <a href="${publication.url}" class="external-link" aria-label="External link to publication"></a>
    </div>
...
// Function to create an experience element
function createExperienceElement(exp) {
  ...
  div.innerHTML = `
    <div class="experience-header">
      <div>
        <a href="${exp.companyLink}" class="experience-title">
          <h3>${exp.title}</h3>
        </a>
        <p class="date">${exp.employmentType} | ${exp.location} | ${exp.startDate} - ${exp.endDate}</p>
      </div>
      <a href="${exp.companyLink}" class="external-link" aria-label="External link to company"></a>
    </div>
...
function createCertificationElement(cert) {
  ...
  div.innerHTML = `
    <div class="certification-header">
      <div>
        <a href="${cert.url}" class="certification-title">
          <h3>${cert.title}</h3>
        </a>
        <p class="date">${cert.issuer}, ${cert.year}</p>
      </div>
      <a href="${cert.url}" class="external-link" aria-label="External link to certificate"></a>
    </div>
...
// Function to load experiences into the DOM
...
// Function to load certifications into the DOM
...
// Ensure that the download button (with id "downloadResumeButton") in the resume section is functional
function setupDownloadButton() {
    ...
    const handleDownload = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch("./documents/Lebenslauf_Sinowski.pdf");
...
document.addEventListener('DOMContentLoaded', () => {
  loadExperiences();
  loadPublications();
  loadCertifications(); // added certifications loading
  initFloatingNav();
  setupDownloadButton();
});
// Publications data (updated with JSON content)
